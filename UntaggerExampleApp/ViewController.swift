...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.frame = CGRect(
            x: 20,
            y: bodyLabel.frame.origin.y + bodyLabel.frame.size.height + 10,
            width: view.frame.size.width - 40 - 110,
            height: 44
        )
        ...
        extractArticleButton.frame = CGRect(
            x: textField.frame.origin.x + textField.frame.size.width + 10,
    ...
    func extractArticle() {
        if let text = self.textField.text, let url = URL(string: text) {
            UntaggerManager.sharedInstance.getText(
                url: url,
                preStripJavascript: true
            ) { [weak self] (title, body, _, error) in
