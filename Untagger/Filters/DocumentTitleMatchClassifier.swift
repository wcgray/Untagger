...
class DocumentTitleMatchClassifier : BaseFilter {
    ...
    init(document: UntaggerDocument, title: String?) {
        ...
        if let title = title, (titleLen > 0) {
            let workingTitle = replaceMatches(source: title, regex: DocumentTitleMatchClassifier.titleRegex, replacement: " ")
            let adjustedTitle = workingTitle.replacingOccurrences(of: "'", with: "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            ...
            addPotentialTitles(title: adjustedTitle, regex: DocumentTitleMatchClassifier.veticalLineSeperator, minWords: 4)
            addPotentialTitles(title: adjustedTitle, regex: DocumentTitleMatchClassifier.dashSeperator, minWords: 4)
            
            let rf1 = replaceFirstRegexMatch(source: adjustedTitle, regex: DocumentTitleMatchClassifier.preDash, replacement: "");
            let rf2 = replaceFirstRegexMatch(source: adjustedTitle, regex: DocumentTitleMatchClassifier.postDash, replacement: "");
    ...
    override func process() -> Bool {
        ...
        for tb in document.textBlocks {
            let workingTitle = replaceMatches(source: tb.text, regex: DocumentTitleMatchClassifier.titleRegex, replacement: " ")
            var adjustedText = workingTitle.replacingOccurrences(of: "'", with: "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            ...
            if let regex = cachedRegex(pattern: "[\\?\\!\\.\\-\\:]+") {
                adjustedText = replaceMatches(source: tb.text, regex: regex, replacement: "").trimmingCharacters(in: .whitespacesAndNewlines)
