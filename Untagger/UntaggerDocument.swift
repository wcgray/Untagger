...
class UntaggerDocument : NSObject, UntaggerHTMLParserDelegate {
    ...
    var title : String = ""
    var didProcessDocument : ((UntaggerDocument) -> Void)? = nil
    ...
    var lastStartTag : String? = nil
    var lastEndTag : String? = nil
    ...
    func recievedCharacters(_ characters: String!) {
        ...
        let whitespaceNormalizedString : String = (characters ?? "").components(separatedBy: .whitespacesAndNewlines).joined(separator: " ")
        
        while (start < characters.count) {
            let c = (whitespaceNormalizedString as NSString).substring(with: NSMakeRange(start, 1))
            if (c == " ") {
        ...
        while (len > 0) {
            ...
            let c = (whitespaceNormalizedString as NSString).substring(with: NSMakeRange(index, 1))
            if (c == " ") {
                endWhitespace = true
                len -= 1
            } else {
        ...
        //let charactersToAppend = (whitespaceNormalizedString as NSString).substring(from: start)
        let charactersToAppend = (whitespaceNormalizedString as NSString).substring(with: NSMakeRange(start, len))
            //whitespaceNormalizedString[whitespaceNormalizedString.index(whitespaceNormalizedString.startIndex, offsetBy: start)...]
    ...
    func extractArticle() {
        let _ = TerminatingBlocksFinder.init(document: self).process()
        let _ = DocumentTitleMatchClassifier.init(document: self, title: self.title).process()
        
        let _ = NumWordsRulesClassifierFilter.init(document: self).process()
        let _ = IgnoreBlocksAfterContentFilter.init(document: self).process()
        let _ = TrailingHeadlineToUntaggerFilter.init(document: self).process()
        
        let maxDistance1 = BlockProximityFusion.maxDistance1(document: self)
        let _ = maxDistance1.process()
        
        let _ = UntaggerBlockFilter.init(document: self, labelToKeep: TITLE).process()
        
        let maxDistance1ContentOnlySameTagLevel = BlockProximityFusion.maxDistance1ContentOnlySameTagLevel(document: self);
        let _ = maxDistance1ContentOnlySameTagLevel.process();
        
        let keepLargestBlockFilter = KeepLargestBlockBlockFilter.expandToSameTagLevelMinWords(document: self)
        let _ = keepLargestBlockFilter.process()
        
        let _ = ExpandTitleToContentFilter.init(document: self).process()
        let _ = LargeBlockSameTagLevelToContentFilter.init(document: self).process()
        let _ = ListAtEndFilter.init(document: self).process()
    }
