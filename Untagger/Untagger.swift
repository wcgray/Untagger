...
public class UntaggerManager {
    ...
    public func getText(htmlString: String, preStripJavascript: Bool = false, _ completion: @escaping ((UntaggerResult) -> Void)) {
    ...
    public func getText(url: URL, preStripJavascript: Bool = false, _ completion: @escaping ((UntaggerResult) -> Void)) {
        let document = UntaggerDocument()
        let parser = UntaggerHTMLParser()
        parser.delegate = document
        self.pendingDocuments[document.id] = document
    ...
    private static func stripScriptTags(_ source: String) -> String {
        ...
        while index < currentSourceCount {
            let nextOpen = (currentSource as NSString).range(of: "<script", options: [], range: NSMakeRange(index, currentSourceCount - index))
            ...
            let nextClose = (currentSource as NSString).range(of: "</script>", options: [], range: NSMakeRange(openStartIndex, currentSourceCount - openStartIndex))
            if nextOpen.location != NSNotFound, nextOpen.location < nextClose.location {
                ...
                if scriptCount > 0 {
                    ...
                    if scriptCount == 0 {
                        ...
                        let sourceWithScriptStripped = (currentSource as NSString).replacingCharacters(in: NSMakeRange(lastOpen.location, sourceWithScriptStrippedLength), with: "")
